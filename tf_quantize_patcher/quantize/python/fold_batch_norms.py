...
from tensorflow.contrib.quantize.python import common, input_to_ops
  ...
  for match in _FindFusedBatchNorms(graph):
    ...
    with graph.as_default(), graph.name_scope(scope + sep):
      with graph.name_scope(scope + sep + 'BatchNorm_Fold' + sep):
        ...
        if is_training and match.bn_op.get_attr('is_training'):
...
def _FindFusedBatchNorms(graph):
  ...
  # conv/matmul -> bn path might have a switch before FusedBatchNorm
  conv_switch_pattern = graph_matcher.OpTypePattern(
      'Switch',
      inputs=[graph_matcher.OneofPattern([layer_output_pattern, matmul_reshape_pattern]),
              graph_matcher.OpTypePattern('*')]
  )
  ...
  batch_norm_pattern = graph_matcher.OpTypePattern(
      ...
      inputs=[
          graph_matcher.OneofPattern(
              [matmul_reshape_pattern, conv_switch_pattern, layer_output_pattern]), gamma_pattern,
  ...
  bn_output_merge = graph_matcher.OpTypePattern(
      'Merge',
      inputs=[graph_matcher.OpTypePattern('*'), batch_norm_pattern],
      ordered_inputs=False
  )

  moving_average_sub_pattern = graph_matcher.OpTypePattern(
      'Sub', inputs=[moving_average_pattern, graph_matcher.OneofPattern([batch_norm_pattern, bn_output_merge])])
  ...
  def _GetLayerMatch(match_result):
    ...
    if is_training:
      ...
      for mul_match_result in moving_avg_mul_matcher.match_graph(graph):
        ...
        merge_op = mul_match_result.get_op(bn_output_merge)
        sub_op_inputs = [sub_op.inputs[1].name] if merge_op is None else [i.name for i in merge_op.inputs]
        if bn_op.outputs[1].name in sub_op_inputs:
        ...
        if bn_op.outputs[2].name in sub_op_inputs:
...
def _FoldUnfusedBatchNorms(graph, is_training, freeze_batch_norm_delay):
  ...
  for bn in common.BatchNormGroups(graph):
    ...
    # The following doesn't apply for our use case
...
def _IsValidUnfusedBatchNorm(graph, context):
  ...
  add_shift = graph.get_operation_by_name(context + 'batchnorm/add_1')
...
def _GetBatchNormParams(graph, context, has_scaling):
  ...
  op_suffix_mean = 'moments/Squeeze'
  op_suffix_variance = 'moments/Squeeze_1'
  op_suffix_epsilon = 'batchnorm/add/y'
  op_suffix_bn_decay_mean = 'AssignMovingAvg/decay'
  op_suffix_bn_decay_var = 'AssignMovingAvg_1/decay'

  use_resource = True  # variable_scope.get_variable_scope().use_resource
  if use_resource:
    op_suffix_gamma = 'gamma/Read/ReadVariableOp'
    op_suffix_moving_variance = (
        'moving_variance/Read/ReadVariableOp')
    op_suffix_moving_mean = ('moving_mean/Read/ReadVariableOp')
  else:
    op_suffix_gamma = 'gamma'
    op_suffix_moving_variance = 'moving_variance'
    op_suffix_moving_mean = 'moving_mean'
...
def _CreateFoldedOp(graph, context, has_scaling, freeze_batch_norm_delay,
  ...
  mul_scale = graph.get_operation_by_name(context + 'batchnorm/' + mul_scale_name)
  ...
  if op_below.type == 'DepthwiseConv2dNative':
    ...
    scale = graph.get_operation_by_name(context + 'batchnorm/' + scale_name)
  ...
  add_shift = graph.get_operation_by_name(context + 'batchnorm/add_1')  # TODO: not sure if correct
...
def _HasScaling(graph, input_to_ops_map, bn):
  ...
  """
  rsqrt_op = graph.get_operation_by_name(bn + 'batchnorm/Rsqrt')
