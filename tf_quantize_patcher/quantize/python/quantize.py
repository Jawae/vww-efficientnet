...
from tensorflow.contrib.quantize.python import common, input_to_ops, quant_ops
...
def Quantize(graph,
  ...
  * Bypass/Post-activation Bypass: Quantize both input and output
  ...
  for layer_match in _FindLayersToQuantize(graph):
    ...
    if layer_match.bias_tensor is not None:
        _InsertQuantOp(
            context,
            'bias_quant',
            # consumer list would be either bias_add_op or activation_op for the last layer
            layer_match.bias_tensor.op, [layer_match.bias_add_op or layer_match.activation_op],
            is_training,
            moving_avg=False,
            ema_decay=ema_decay,
            quant_delay=quant_delay,
...
def _FindLayersToQuantize(graph):
  ...
  # Support a simplified set of bias additions --- not sure yet how all options would look like
  bias_var_pattern = graph_matcher.OpTypePattern('Variable|VariableV2')
  bias_resource_var_pattern = graph_matcher.OpTypePattern('ReadVariableOp')
  ...
  folded_bias_add_pattern = graph_matcher.OpTypePattern(
      'Add',
      inputs=[
          graph_matcher.OneofPattern([layer_output_pattern, post_layer_op_correction_pattern]),
          graph_matcher.OpTypePattern('*')
      ],
      ordered_inputs=False)
  ...
  # incorrectly.
  batch_norm_identity = graph_matcher.OneofPattern([
      graph_matcher.OpTypePattern('Identity', inputs=[folded_bias_add_pattern]),
      graph_matcher.OpTypePattern('Merge', inputs=[folded_bias_add_pattern, '*'], ordered_inputs=False)])
  ...
  bias_add_pattern = graph_matcher.OpTypePattern(
      ...
      inputs=[
          ...
          graph_matcher.OneofPattern([bias_var_pattern, bias_resource_var_pattern])
      ],
      ordered_inputs=False)
  ...
  # TODO: only apply during training
  dropout_pattern = graph_matcher.OpTypePattern(
      'Multiply', inputs=[graph_matcher.OpTypePattern('*'), activation_pattern], ordered_inputs=False
  )

  post_activation_bypass_pattern = graph_matcher.OpTypePattern(
      'Add', inputs=[graph_matcher.OpTypePattern('*'),
                     graph_matcher.OneofPattern([activation_pattern, dropout_pattern])],
      ordered_inputs=False)
  ...
  for match_result in post_activation_bypass_layer_matcher.match_graph(graph):
    ...
    bias_tensor = match_result.get_tensor(bias_var_pattern)
    if bias_tensor is None:
        bias_tensor = match_result.get_tensor(bias_resource_var_pattern)
    ...
    if layer_op not in matched_layer_set:
      ...
      layer_matches.append(
          _LayerMatch(layer_op, weight_tensor, activation_op, bypass_op,
                      post_activation_bypass_op, bias_add_op, bias_tensor))
  ...
  for match_result in final_layer_matcher.match_graph(graph):
    ...
    if layer_op not in matched_layer_set:
      matched_layer_set.add(layer_op)
      layer_matches.append(
          _LayerMatch(layer_op, weight_tensor, activation_op, None, None, None, bias_tensor))
  ...
  for match_result in sep_conv_matcher.match_graph(graph):
    ...
    if layer_op not in matched_layer_set:
      matched_layer_set.add(layer_op)
      layer_matches.append(
          _LayerMatch(layer_op, weight_tensor, activation_op, None, None, None, None))
